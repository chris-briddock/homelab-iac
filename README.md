...
- `pki.tf` — documents the file-based PKI model; the actual root/intermediate CA
  live in `pki/` as local files minted once by `scripts/gen-pki.sh` (keys are
  git-ignored, certs committed), consumed by step-ca and every VM's trust anchor.
...
encryption (PBKDF2 -> AES-GCM) because they hold generated service passwords
(the CA private keys are local files now — see the PKI entry in Known gotchas —
but the encrypted intermediate key still transits plan/state on its way to
ca-vm). Both are configured in `versions.tf`.
...
- Web services sit behind per-VM Caddy, which requests certs from step-ca over
  ACME using the internal root CA (see `pki.tf` / `outputs.tf: root_ca_pem`).
...
- **File-based PKI (no keys in tfstate)**: the root + intermediate CA are minted
  ONCE by `scripts/gen-pki.sh` (idempotent, openssl via a container; never
  overwrites existing files) into `pki/`:
  - Certs (`pki/*.crt`) are committed — they're public by definition; the root
    cert (`pki/root-ca.crt`) is every VM's trust anchor (embedded in each
    cloud-init at `/etc/pki/ca-trust/source/anchors/lab-internal-root-ca.crt`;
    caddy-fronted VMs also get `/etc/infra/root_ca.crt`, the dns VMs
    `/etc/caddy/root_ca.crt`).
  - Private keys (`pki/*.key`) are git-ignored and live ONLY on the operator's
    machine — back them up somewhere safe outside this repo. No private key
    material is generated by or stored in tofu anymore.
  - On a fresh clone, run `scripts/gen-pki.sh` once before `tofu apply`;
    without `pki/*.key` the ca module's plan/apply WILL fail — that is
    expected on a machine without the keys.
  - **`pki/root-ca-cert.pem` is the DEAD root**: its key lived only in a prior
    tfstate that was lost — the cert is kept for reference only. NEVER
    reintroduce it as a trust anchor; `pki/root-ca.crt` replaced it.
  - Rotating the root cert changes every embedding VM's cloud-init, so those
    VMs must be re-rolled (see "Rolling out config changes"). Choosing the
    re-roll MODE per VM (destructive disk replace vs cloud-init-only) matters:
    - **nfs does NOT embed the root CA at all** — no re-roll for PKI. It also
      holds the git repos as a `/srv/nfs` bind mount on its root disk, so NEVER
      disk-replace it during a PKI rotation; you'd wipe the repos.
    - **postgres + surrealdb embed the OS trust anchor but hold live data in
      named volumes on the root disk** (giteadb lives in `postgres-data`) —
